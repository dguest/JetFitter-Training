                                   ...
                                   TString likelihoodCoefDir="../trainingResultsLikelihood")
{
  ...
  //a single file for all jet collections...
...
//  calibrationFileToWrite+=*collectionsToProcessIter;
  ...
  vector<TString> collectionsToProcess=getCollectionsToPutIntoFile();
...
//    }
    ...
    TString jetName=calibrationFileToTake;
    jetName.ReplaceAll("Jets","");
    jetName.ReplaceAll("AOD","");

    gDirectory->mkdir("JetFitter");
    gDirectory->cd("JetFitter");

    gDirectory->mkdir(jetName);
    gDirectory->cd(jetName);
    ...
    TString calibrationFileToReallyTake=renameForCalibration(calibrationFileToTake);

    std::cout << " Before renaming: " << calibrationFileToTake << " after " << calibrationFileToReallyTake << std::endl;

    TString histoDirBase=mainDirectoryName+"/"+calibrationFileToReallyTake+"/";

    //now first work on combined with IP3
    gDirectory->cd("/JetFitter/"+jetName+"/"+tagToUse);
    ...
    calibrationFile->cd("/JetFitter/"+jetName+"/"+tagToUse+"/comb/");
    ...
    calibrationFile->cd("/JetFitter/"+jetName+"/"+tagToUse+"/standalone/");
...
//likelihoodCoefDir
    ...
    if (withLikelihood)
    {
      TString histoDirBasePDFs=likelihoodPDFDir+"/"+calibrationFileToReallyTake+"/trainingResultsLikelihood/smoothedhistos_"+calibrationFileToReallyTake+"_";
//      gDirectory->cd("/"+tagToUseForLikelihood);
      ...
      for (std::vector<TString>::const_iterator iterFlavour=flavours.begin();iterFlavour!=flavours.end();++iterFlavour)
      {
        ...
        do
        {
          ...
          if (next==0) break;
//          std::cout << " class name: " << gDirectory->Get(next->GetName())->ClassName() << " Name: " << gDirectory->Get(next->GetName())->GetName() << std::endl;
          if (gDirectory->Get(next->GetName())->ClassName()==TString("TH1F"))
          {
            ...
            calibrationFile->cd("/JetFitter/"+jetName+"/"+tagToUseForLikelihood);
            myHisto->Write();
          } else 
          {
      ...
      TString histoDirBaseCoef=likelihoodCoefDir+"/coefficientsHistos_"+calibrationFileToReallyTake;
      gDirectory->cd("/JetFitter/"+jetName+"/"+tagToUseForLikelihood);
